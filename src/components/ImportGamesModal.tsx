...
export function ImportGamesModal({ isOpen, onClose }: ImportGamesModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#1a2744] rounded-2xl border border-[#3a4a6e] w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl">
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-[#3a4a6e]">
                    <h2 className="text-xl font-bold text-white">Import Game</h2>
                    <button 
                        onClick={onClose}
                        className="p-2 hover:bg-[#243354] rounded-lg text-[#a8b4ce] hover:text-white transition-colors"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="flex border-b border-[#3a4a6e]">
                    ...
                    <button
                        ...
                        className={`flex-1 py-3 px-4 text-sm font-bold transition-colors ${
                            platform === 'chesscom' 
                                ? 'text-[#69e0a3] bg-[#243354] border-b-2 border-[#69e0a3]' 
                                : 'text-[#a8b4ce] hover:text-white hover:bg-[#243354]/50'
                        }`}
                    >
                        ♟ Chess.com
                ...
                <div className="p-5">
                    <div className="flex flex-col sm:flex-row gap-3">
                        <input
                            ...
                            className="flex-1 bg-[#0f1729] border border-[#3a4a6e] rounded-lg px-4 py-3 text-white placeholder:text-[#6b7a99] focus:outline-none focus:border-[#5ec2f2] transition-colors"
                        />
                        <button
                            onClick={handleFetchGames}
                            disabled={loading || !username.trim()}
                            className="px-6 py-3 bg-[#5ec2f2] hover:bg-[#7fd0f7] disabled:bg-[#3a4a6e] disabled:text-[#6b7a99] text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
                        >
                            {loading ? (
                        ...
                        <div className="space-y-2">
                            ...
                            {games.map((game) => (
                                <button
                                    ...
                                    className="w-full bg-[#243354] hover:bg-[#2d3d5e] border border-[#3a4a6e] rounded-lg p-4 text-left transition-colors group"
                                >
                                    <div className="flex items-center justify-between">
                                        ...
                                        <ChevronRight size={20} className="text-[#6b7a99] group-hover:text-[#5ec2f2] transition-colors shrink-0" />
                                    </div>
                                </button>
                            ))}
                        </div>
